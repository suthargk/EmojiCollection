...
export default memo(function EmojiHubItem({ emoji, onSelectEmojiHandler }) {
  ...
  return (
    <li
      className="emoji-hub-set-item"
      onClick={() => {
        navigator.clipboard.writeText(emojiObj.convertedEmoji);
        onSelectEmojiHandler(emojiObj);
      }}
    >
      <span className="emoji-hub-set-item--copied">copied</span>
